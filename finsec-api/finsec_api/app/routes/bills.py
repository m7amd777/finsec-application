    ...
    try:
        ...
        for bill in bills:
            try:
                ...
                print(f"Error formatting bill {bill.id}: {str(e)}")
                continue
...
def pay_bill():
    ...
    try:
        ...
        bill_amount = round(float(bill.amount), 2)
        print(f"Comparing amounts - Payment: {payment_amount} ({type(payment_amount)}), Bill: {bill_amount} ({type(bill_amount)})")
        ...
        if payment_amount != bill_amount:
            print(f"Error: Amount mismatch - Payment: {payment_amount}, Bill: {bill_amount}")
            return jsonify({
                'error': 'Invalid amount',
